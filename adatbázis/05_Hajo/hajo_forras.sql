...
--1.
select honan hova indul erkezik
from hajo_menetrend
where jarat ='J1';
--2.
select erkezik
from hajo_menetrend
where hova='Balatonfüred' indul='11:30:00 between 12:30:00';
--3.
select jarat
from hajo_menetrend
where erkezik = (select erkezik (max desc hova ='Balatonföldvár')from adatok);

--5.
select jarat
from hajo_menetrend
where indul =( select indul max (desc limit 1)from hajo_menetrend)from adatok);
--6.
select honnan
from hajo_menetrend
where jarat='A4';
...
select jarat
from hajo_menetrend
where erkezik LIKE'%SIÓFOK%';

--8.
select hova
from hajo_menetrend
where erkezik = (select erkezik max desc limit 1);
...
select jarat
from hajo_menetrend
where indul >= 11:00:00;

--13.
select jarat
from hajo_menetrend
where erkeznek >= 17:00:00;

--14.
select honnan
from hajo_menetrend
where indul (select indul min asc limit 1 ) from adatok);
--15.
select jarat
from hajo_menetrend
where indul ="11:00:00";
